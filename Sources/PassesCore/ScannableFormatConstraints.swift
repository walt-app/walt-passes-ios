...
enum ScannableFormatConstraints {
    ...
    ///
    /// **Deliberately one byte above Android's 2330** (ios-pjs.20, human-approved
    /// 2026-08-17): Android's encoder emits a UTF-8 ECI header whose 12 bits cost one
    /// byte of v40-M capacity; the CoreImage generator emits no ECI, so iOS keeps the
    /// full spec-table 2331. `qrByteModeCeilingIsExactAtTheBoundary` pins the number
