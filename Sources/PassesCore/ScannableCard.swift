...
/// validator (length caps, charset rules, bidi/control-character rejection). The memberwise
/// initializer is `internal` (Swift's default for a `public` struct), so this invariant cannot
/// be bypassed by an outside caller hand-building one around raw input — the only construction
/// path is via the validator's `ScannableCardCreateResult.success`.
///
