...
struct ManifestVerifierTests {
    ...
    @Test func validManifestPasses() {
        ...
        let manifest = entry(MANIFEST_FILE_NAME, "{\"pass.json\":\"\(PkpassFixtures.sha1Hex(pass.1))\"}")
    ...
    @Test func invalidJsonManifest() {
        let result = verifyManifest([entry(MANIFEST_FILE_NAME, "not json")])
        #expect(result == .failed(.invalidJson))
    }

    @Test func nonStringValueIsInvalidShape() {
        let result = verifyManifest([entry(MANIFEST_FILE_NAME, "{\"pass.json\": 5}")])
        #expect(result == .failed(.invalidShape))
    }

    @Test func arrayTopLevelIsInvalidShape() {
        let result = verifyManifest([entry(MANIFEST_FILE_NAME, "[]")])
        #expect(result == .failed(.invalidShape))
    }

    @Test func selfReferentialSignatureEntry() {
        let manifest = entry(MANIFEST_FILE_NAME, "{\"signature\":\"\(String(repeating: "a", count: 40))\"}")
        #expect(verifyManifest([manifest]) == .failed(.selfReferentialEntry))
    }

    @Test func invalidHashFormat() {
        let manifest = entry(MANIFEST_FILE_NAME, "{\"pass.json\":\"xyz\"}")
        #expect(verifyManifest([manifest]) == .failed(.invalidHashFormat(entryName: "pass.json")))
    }

    @Test func missingEntry() {
        let manifest = entry(MANIFEST_FILE_NAME, "{\"pass.json\":\"\(String(repeating: "a", count: 40))\"}")
    ...
    @Test func hashMismatch() {
        ...
        let manifest = entry(MANIFEST_FILE_NAME, "{\"pass.json\":\"\(wrong)\"}")
    ...
    @Test func mixedCaseHexAccepted() {
        ...
        let manifest = entry(MANIFEST_FILE_NAME, "{\"pass.json\":\"\(hex)\"}")
